...
namespace DNA_Test.Scheduler
{
    public partial class SchedulerForm : Form
    {
        ...
        private void button_InsertEndpoints_Click(object sender, EventArgs e)
        {
            try
            {
                Excel.Range selection = MyAddin.MyApp.Selection;
                ...
                int points = scheduler.GetEndpoints().Length;
                if (points > 10000)
                {
                ...
                for (int i = 0; i < Math.Min(selection.Cells.Count, scheduler.GetEndpoints().Count()); i++)
                {
                    if (isVertical)
                        cellValues[i, 0] = scheduler.GetEndpoints()[i].ToOADate().ToString();
                    else
                        cellValues[0, i] = scheduler.GetEndpoints()[i].ToOADate().ToString();
                }
        ...
        private void button_InsertMidpoints_Click(object sender, EventArgs e)
        {
            try
            {
                Excel.Range selection = MyAddin.MyApp.Selection;

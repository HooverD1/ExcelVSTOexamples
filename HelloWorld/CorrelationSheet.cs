...
namespace HelloWorld
{
    ...
    {
        public CorrelationMatrix CorrelMatrix { get; set; }     //things that are correlated - estimates and inputs
        public Excel.Range MatrixTopLeft { get; set; }
        public Excel.Worksheet CorrelWorkSheet { get; set; }
        ...
        public CorrelationSheet() : base()
        {
            ...
            MatrixTopLeft = ThisWorkSheet.Range["H4"];
        ...
        public void PrintCorrelationMatrix()        //this should be within a sheet object I think..
        {
            ...
            for(int row = 0; row < columns; row++)
            {
                for (int col = 0; col < columns; col++)
                {
                    ...
                    printCell.Offset[row, col].Value = CorrelMatrix.Correlations[row,col].Coefficient;
                }
            }
            //CorrelMatrix.FormatMatrix(printCell);
            for (int row = 0; row < columns; row++)
            {
                Excel.Range header = MatrixTopLeft.Offset[-1, row];
                if(row>0)
                    header.Orientation = Excel.XlOrientation.xlUpward;
                //header.Value = CorrelMatrix.Correlations[0, row].input2.Name;
                //MatrixTopLeft.Offset[row, row].Value = CorrelMatrix.Correlations[row, 0].input1.Name;
            }
        ...
        private int GetCorrelationColCount()
        {
            int sum = 0;
            int this_sum = 0;
            double correlationCount = this.CorrelMatrix.Correlations.GetLength(0) * this.CorrelMatrix.Correlations.GetLength(1);
        ...
        public bool CheckTransitivity()
        {
            ...
            //foreach(Correlation correl in CorrelMatrix.Correlations)
            //{
        ...
        {
            exportCell.Value = this.CorrelMatrix.CreateCorrelString();
        }
